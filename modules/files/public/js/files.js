	...
	this.addMouseEvents = function() {
		...
		$("#contextmenu .menu li").on('click', function(e) {
			...
			switch (action) {
				...
				case 'download':
					FileModel.download(FileModel.list.getAllSelected());
					break;
...
var FileView = new function() {
	...
	this.init = function(view) {
		self.view = (view && view != "public") ? view : "files";
		self.setView(view);
		self.enableLazyLoad();
	...
	this.setImgthumbnail = function(index, requestID) {
		...
		if (item != null && requestID == FileModel.requestID && self.view != 'trash') {
			...
			if (thumbnail && thumbnail.style.backgroundImage == '' && visible && (item.type == 'image' || item.type == 'pdf')) {
				...
				img.src = encodeURI("api/files/get?targets=" + JSON.stringify([item.id]) + "&width=" + size + "&height=" + size + "&thumbnail=1&token=" + Util.getToken());
				img.onload = function() {
...
var FileModel = new function() {
	...
	this.download = function(targets) {
		var bId = Util.startBusy();
		var folderSel = false;
		var ids = [];
		var count = Object.keys(targets).length;
		...
		for (var elem in targets) {
			ids.push(targets[elem].id);

			if (targets[elem].type == "folder") {
				folderSel = true;
			}
		}

		if (folderSel || count > 1) {
		...
		$.ajax({
			url: 'api/files/get',
			type: 'get',
			data: {targets: JSON.stringify(ids)}
		}).done(function(data, statusText, xhr) {
			Util.download('api/files/get', {targets: JSON.stringify(ids)});
		}).fail(function(xhr, statusText, error) {
			Util.notify(xhr.statusText, true, true);
		}).always(function() {
			Util.endBusy(bId);
	...
	this.loadPublic = function() {
		...
		if (self.downloadPub) {
			self.download(self.downloadPub);
			return;
		}
	...
	this.openPDF = function(id) {
		window.location.href = 'api/files/get?targets=' + JSON.stringify([id])+ '&token=' + Util.getToken();
	}
