...
class Backup {
	...
	public function __construct($token) {
		...
		$this->credentials		= dirname(__FILE__) . '/../config/googledrive/client_secret.json';
		$this->token			= dirname(__FILE__) . '/../config/googledrive/access-token_' . $this->username . '.json';
		$this->temp				= $this->config['datadir'] . $this->username . "/.tmp/";
	...
	public function start() {
		...
		if (file_exists($this->lock)) {
			header('HTTP/1.1 500 Backup already running');
			return null;
		}

		// Check if connected to the internet
		if (!@fopen('http://google.com', 'r')) {
			header('HTTP/1.1 500 No internet connection');
			return null;
		}

		// Check if credentials file exists
		if (($access_token = $this->read_access_token()) === null && $this->refresh_token() === false) {
			header('HTTP/1.1 500 Could not read access token');
			return null;
		}
	...
	public function cancel() {
		...
		header('HTTP/1.1 Could not remove lock');
		return null;
	}
	...
	public function disable() {
		...
		header('HTTP/1.1 500 Could not disable backup');
		return null;
	}
	...
	public function create_folder($title, $parent_id = "root") {
		$access_token = $this->read_access_token();
	...
	public function create_google_file($path, $parent_id, $description) {
		$access_token = $this->read_access_token();
	...
	public function upload($path, $parent_id = "root", $description = "") {
